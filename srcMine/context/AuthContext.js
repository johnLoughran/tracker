...
const signup = ( dispatch ) => async ({ email, password }) => {
  ...
  try {
    ...
    dispatch({ type: 'signupOrSignin', payload: response.data.token }); // calls the switch case above, resets state
...
// impliit return syntax
const signin = ( dispatch ) => async ({ email, password }) => {
  // Try to sign in
  // Handle success by updating state
  try {
    // wait for a response from a post to the signin route
    const response = await trackerApi.post( '/siginin', { email, password } );
    // Set an item in async storage to be the token from the response
    await AsyncStorage.setItem('token', response.data.token );
    // dispatch this token to the signin action
    dispatch({ type: 'signupOrSignin', payload: response.data.token });
    // nav to TrackList (if no errors above)
    navigate('TrackList');
  }
  catch (err) {
    console.log(err);
